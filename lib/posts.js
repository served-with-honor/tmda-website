...
export async function getPostBySlug(slug) {
  ...
  try {
    postData = await fetchWP(QUERY_POST_BY_SLUG, {
      variables: {
        slug,
      },
    });
  ...
  if (process.env.WORDPRESS_PLUGIN_SEO === true) {
    try {
      seoData = await fetchWP(QUERY_POST_SEO_BY_SLUG, {
        variables: {
          slug,
        },
      });
...
export async function getAllPosts(options = {}) {
  ...
  const data = await fetchWP(allPostsIncludesTypes[queryIncludes], {
  });
...
export async function getPostsByAuthorSlug({ slug, ...options }) {
  ...
  try {
    postData = await fetchWP(postsByAuthorSlugIncludesTypes[queryIncludes], {
      variables: {
        slug,
      },
    });
...
export async function getPostsByCategoryId({ categoryId, ...options }) {
  ...
  try {
    postData = await fetchWP(postsByCategoryIdIncludesTypes[queryIncludes], {
      variables: {
        categoryId,
      },
    });
...
export async function getPostsPerPage() {
  ...
  try {
    ...
    const { data } = await fetchWP(QUERY_POST_PER_PAGE, {
    });
