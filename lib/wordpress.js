...
async function getPostMetaData(id, idType) {
  // TODO - handle slugs or ids
	try {
		const postUrl = `${constants.wordpress.base}${constants.wordpress.posts}/${id}`;
    const response = await fetch(`${constants.wordpress.base}${constants.wordpress.rankmath}?url=${postUrl}`);
