...
export default function Custom3ActionSection({ items, name = 'Custom 3 Action Section'}) {
    ...
    function handleCTAClick(urlLink){
        debugger
    }
    ...
    return (
        <Grid container sx={{minWidth: '100%'}}>
            {items.map(({icon, heading, description, urlLink, buttonLabel}, index) => {
                return(
                    <Paper onClick={() => handleCTAClick(urlLink)} sx={{m: 5, width: '25%', minHeight: '300px', borderRadius: 0, backgroundColor: 'secondary.100'}}>
                        <Stack sx={{alignItems: 'center', pt: 5}} direction='column'>
                            ...
                            <Button variant={'outlined'} sx={{mb: 5}}>{buttonLabel}</Button>
                        </Stack>
                    </Paper>
                )
            })}
